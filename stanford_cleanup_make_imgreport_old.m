...
corr_string=['Corrected Core: ' num2str(COREvol_corr,'%2.1f')  ' mL.  Corrected MAX>6: '  num2str(HYPOvol_corr,'%2.1f')  ' mL.  Corrected Tmax>10: '  num2str(HYPOvol10_corr,'%2.1f') '  ' identifier ];


%REPLACE this:
summaryimg_rgb_orig=textonrgb(summaryimg_rgb_orig,orig_string,[1 1 1],[0 0 0],0,50);
summaryimg_rgb_corr=textonrgb(summaryimg_rgb_corr,corr_string,[1 1 1],[0 0 0],0,50);

%with (repeat for orig and corrected:
figure; image(summaryimg_rgb_orig); text(,,orig_string)
..getframe(gca)
%then just call that summaryimg_rgb_orig and summaryimg_rgb_corr